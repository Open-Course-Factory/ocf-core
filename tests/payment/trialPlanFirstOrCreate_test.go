...
// Tests that EnsureFreePlanExists is idempotent and uses FirstOrCreate to avoid
...
// EnsureFreePlanExists multiple times results in exactly one Trial plan in the DB.
func TestEnsureTrialPlanExists_FirstOrCreate_Idempotent(t *testing.T) {
	...
	err := db.Where("name = ? AND price_amount = 0", services.FreePlanName).Find(&plans).Error
...
// no Trial plan exists, EnsureFreePlanExists creates one with the correct fields.
func TestEnsureTrialPlanExists_FirstOrCreate_CreatesWhenMissing(t *testing.T) {
	db := freshTestDB(t)

	// Precondition: no Trial plan exists
	var count int64
	db.Model(&models.SubscriptionPlan{}).Where("name = ?", services.FreePlanName).Count(&count)
	require.Equal(t, int64(0), count, "no Trial plan should exist before calling the function")

	initialization.EnsureFreePlanExists(db)

	var plan models.SubscriptionPlan
	err := db.Where("name = ? AND price_amount = 0", services.FreePlanName).First(&plan).Error
	require.NoError(t, err, "Trial plan should exist after EnsureFreePlanExists")
	...
	assert.Equal(t, 256, plan.MaxMemoryMB, "Trial plan budget = 1 XS (256 MiB)")
}
