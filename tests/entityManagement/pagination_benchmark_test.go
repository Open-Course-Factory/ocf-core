...
func BenchmarkOffsetPagination_FirstPage(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, err := repo.GetAllEntities(BenchmarkEntity{}, 1, 20, map[string]any{}, nil)
		if err != nil {
...
func BenchmarkOffsetPagination_Page250(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		_, _, err := repo.GetAllEntities(BenchmarkEntity{}, 250, 20, map[string]any{}, nil)
		if err != nil {
			b.Fatalf("Error: %v", err)
		}
	}
...
func BenchmarkOffsetPagination_Page500(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		_, _, err := repo.GetAllEntities(BenchmarkEntity{}, 500, 20, map[string]any{}, nil)
		if err != nil {
			b.Fatalf("Error: %v", err)
		}
	}
...
func BenchmarkCursorPagination_FirstPage(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, _, _, err := repo.GetAllEntitiesCursor(BenchmarkEntity{}, "", 20, map[string]any{}, nil)
		if err != nil {
			b.Fatalf("Error: %v", err)
		}
	}
...
func BenchmarkCursorPagination_Page250(b *testing.B) {
	...
	for page := 0; page < 250; page++ {
		_, nextCursor, _, _, err := repo.GetAllEntitiesCursor(BenchmarkEntity{}, cursor, 20, map[string]any{}, nil)
		if err != nil {
	...
	for i := 0; i < b.N; i++ {
		...
		_, _, _, _, err := repo.GetAllEntitiesCursor(BenchmarkEntity{}, cursor, 20, map[string]any{}, nil)
		if err != nil {
			b.Fatalf("Error: %v", err)
		}
	}
...
func BenchmarkSequentialTraversal_Offset(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		for page := 1; page <= 50; page++ {
			_, _, err := repo.GetAllEntities(BenchmarkEntity{}, page, 20, map[string]any{}, nil)
			if err != nil {
...
func BenchmarkSequentialTraversal_Cursor(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		cursor := ""
		for {
			_, nextCursor, hasMore, _, err := repo.GetAllEntitiesCursor(BenchmarkEntity{}, cursor, 20, map[string]any{}, nil)
			if err != nil {
				b.Fatalf("Error: %v", err)
			}
			if !hasMore {
