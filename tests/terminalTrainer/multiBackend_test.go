...
func TestGetTerminalSessionsByUserIDAndOrg(t *testing.T) {
	...
	mkTerminal := func(sessionID string, state models.TerminalState, orgID *uuid.UUID, backend string) *models.Terminal {
	...
	require.NoError(t, db.Create(mkTerminal("session-org1-a", models.StateRunning, &orgID1, "local")).Error)
	require.NoError(t, db.Create(mkTerminal("session-org2-a", models.StateRunning, &orgID2, "cloud1")).Error)
	require.NoError(t, db.Create(mkTerminal("session-no-org", models.StateRunning, nil, "")).Error)
	...
	t.Run("active only filter works with org", func(t *testing.T) {
		require.NoError(t, db.Create(mkTerminal("session-org1-stopped", models.StateStopped, &orgID1, "local")).Error)
...
// NOTE: TestTerminalBackendFieldPersistence was DELETED. It exercised only the
// GORM happy path of saving/retrieving Backend + OrganizationID fields —
// anti-pattern #3 (testing the framework). The CreateTerminalSession
